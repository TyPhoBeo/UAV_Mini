...
# camera_stream chi import stdlib o muc module (cv2/requests nam trong _run),
# nen dong nay AN TOAN ke ca khi may chua cai OpenCV: tab Camera se bao thieu
# thu vien luc bam Connect thay vi lam ca console khong mo duoc.
#
# Duong dan: camera_stream.py nam CUNG thu muc tools/. Chay bang
# `python tools/uav_udp_console.py` thi tools/ da o sys.path[0]; chay tu noi
# khac thi dong sys.path duoi day lo cho truong hop do.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from camera_stream import CameraStreamWorker, STATE_STREAMING  # noqa: E402
from color_detect import ColorDetector, DEFAULT_PROFILES  # noqa: E402
...
# ---- TELEMETRY_LEVEL = 1 (MINIMAL) ----
# Firmware o muc nay chi phat SAU truong. Dong do KHONG khop STATUS_RE (thieu
# gan het), nen phai co regex rieng, neu khong GUI se mu hoan toan va nguoi
# dung tuong mat ket noi.
#
# `fullmatch` chu khong phai `match`: dong FULL cung bat dau bang
# "ARM=1 THR=..." nen `match` se khop CA dong FULL va an mat no.
COMPACT_RE = re.compile(
    r"ARM=(\d) THR=(\d+) R=([-\d.]+) P=([-\d.]+) ALTm=([-\d.]+) TGT=([-\d.]+)\s*"
)
...
class CameraPanel:
    """Panel video MJPEG. Doc anh tu CameraStreamWorker (thread rieng).

    NGUYEN TAC: panel nay KHONG BAO GIO cham socket. Worker doc mang tren
    thread cua no; o day chi hoi latest_frame() moi lan ve. Do la ly do GUI
    khong bao gio dung hinh khi UAV mat Wi-Fi.

    Ve anh: ma hoa PPM roi nap thang vao tk.PhotoImage. KHONG dung Pillow --
    xem so do duoc trong _to_photo(): 0.30 ms/frame, thua suc cho 12 fps.
    """

    RETRY_S = 1.0
    REDRAW_MS = 33          # ~30Hz tran ve; nguon that van la FPS cua UAV

    def __init__(self, parent, notebook, tab_frame, host_getter):
        self._notebook = notebook
        self._tab = tab_frame
        self._host_getter = host_getter
        self._worker = None
        self._photo = None       # PHAI giu tham chieu, khong Tk se GC mat anh
        self._last_seq = -1
        self._deps_ok = False
        self._deps_error = ""
        ...
        self.disconnect_btn = ttk.Button(bar, text="Disconnect", command=self.disconnect,
                                          state=tk.DISABLED)
        self.disconnect_btn.pack(side=tk.LEFT, padx=(4, 12))
        ...
        # ---- Hang thu hai: nhan dien mau ----
        # Tach hang rieng thay vi nhet chung: hang tren la "co ket noi khong",
        # hang duoi la "nhin thay gi" -- hai cau hoi khac nhau.
        bar2 = ttk.Frame(parent)
        bar2.pack(side=tk.TOP, fill=tk.X, padx=6, pady=(0, 4))

        self.detect_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(bar2, text="Nhan dien mau", variable=self.detect_var,
                        command=self._on_detect_toggle).pack(side=tk.LEFT)

        self.mask_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(bar2, text="Xem mask", variable=self.mask_var).pack(
            side=tk.LEFT, padx=(8, 12))

        # Bat/tat tung mau: chinh nguong HSV bang mat de hon nhieu so voi doan.
        self.color_vars = {}
        for prof in DEFAULT_PROFILES:
            v = tk.BooleanVar(value=True)
            self.color_vars[prof.name] = v
            ttk.Checkbutton(bar2, text=prof.name, variable=v,
                            command=self._on_colors_changed).pack(side=tk.LEFT, padx=2)

        ttk.Label(bar2, text=" mask:").pack(side=tk.LEFT, padx=(12, 0))
        self.mask_color_var = tk.StringVar(value=DEFAULT_PROFILES[0].name)
        ttk.Combobox(bar2, textvariable=self.mask_color_var, width=6,
                     state="readonly",
                     values=[p.name for p in DEFAULT_PROFILES]).pack(side=tk.LEFT)

        self.detect_var_text = tk.StringVar(value="")
        tk.Label(parent, textvariable=self.detect_var_text, anchor="w",
                 font=("Consolas", 9), fg="#0057b3").pack(
        ...
        self._msg_id = self.canvas.create_text(
            10, 10, anchor="nw", fill="#888888", font=("Consolas", 10),
            text="Chua ket noi. Bam Connect.")

        parent.after(self.REDRAW_MS, self._redraw)

    # --------------------------------------------------------------- detect
    def _on_detect_toggle(self):
        # Bat/tat processor NGAY tren worker dang chay: khong can ket noi lai.
        if self._worker:
            self._worker.processor = self._process if self.detect_var.get() else None
        if not self.detect_var.get():
            self._last_dets = []
            self.detect_var_text.set("")

    def _on_colors_changed(self):
        for name, var in self.color_vars.items():
            self._detector.enabled[name] = bool(var.get())

    def _process(self, frame):
        """Chay TREN THREAD WORKER, ngay sau khi giai ma. Khong cham Tk o day.

        Moi thu Tkinter phai o GUI thread; goi tu day se hong kieu ngau nhien
        va rat kho tim. Nen ham nay chi tra du lieu, con ve va hien chu la
        viec cua _redraw_once().
        """
        return self._detector.detect(frame)

    # ------------------------------------------------------------------ deps
    def _ensure_deps(self):
        """Kiem ba thu vien bat buoc. Goi mot lan, nho ket qua."""
        if self._deps_ok or self._deps_error:
            return
        try:
            ...
            self._deps_error = ("Thieu thu vien: %s\n"
                                "Chay:  pip install -r requirements-camera.txt" % exc.name)
            return
        self._deps_ok = True
    ...
    def _url(self):
        ...
        return "http://%s:%s/stream" % (host, port)

    def connect(self):
        self._ensure_deps()
        if self._deps_error:
            self._show_message(self._deps_error)
            return
        host = self._host_getter()
        if not host:
            self._show_message("Chua co IP UAV. Dien o o Host tren cung roi bam Connect.")
            return
        if self._worker:
            self.disconnect()
        self._worker = CameraStreamWorker(
            self._url(), retry_s=self.RETRY_S,
            processor=self._process if self.detect_var.get() else None)
        self._worker.start()
    ...
    def disconnect(self):
        ...
        self.status_label.configure(fg="gray")
        self._show_message("Da ngat.")

    def close(self):
        """Goi khi dong app -- thread la daemon nhung van nen dung tu te."""
        if self._worker:
            self._worker.stop(join_s=0.5)
            self._worker = None

    # ------------------------------------------------------------------- ve
    def _show_message(self, text):
        self.canvas.delete("frame")
        self.canvas.itemconfigure(self._msg_id, text=text)

    def _is_visible(self):
        try:
            return self._notebook.nametowidget(self._notebook.select()) is self._tab
        except Exception:
            return True
    ...
    def _redraw(self):
        try:
            ...
            # after() PHAI duoc dat lai ke ca khi ve loi, neu khong panel chet
            # im lang va khong bao gio ve lai nua.
            self.canvas.after(self.REDRAW_MS, self._redraw)
    ...
    def _redraw_once(self):
        ...
        colors = {"STREAMING": "#0a7d2c", "CONNECTING": "#b35c00",
                  "RECONNECTING": "#b35c00", "DISCONNECTED": "gray"}
        # latest_result() tra anh VA ket qua cua DUNG anh do -- khong dung
        # latest_frame() rieng roi doc ket qua rieng, vi hai lan doc co the roi
        # vao hai khung khac nhau va overlay se le mot frame.
        frame, dets, seq = w.latest_result()
        _, _, age = w.latest_frame()
        self._last_dets = dets or []

        if w.state == STATE_STREAMING and frame is not None:
            h, wd = frame.shape[0], frame.shape[1]
            self.status_var.set("STREAMING  %dx%d  %.1f fps  tuoi %.0fms  frames %d  "
                                "decode-fail %d" % (wd, h, w.fps, age * 1000,
                                                     w.frames, w.decode_fails))
        else:
            extra = ("  (%s)" % w.last_error[:60]) if w.last_error else ""
            self.status_var.set("CAMERA %s%s" % (w.state, extra))
        self.status_label.configure(fg=colors.get(w.state, "gray"))

        if self.detect_var.get():
            if getattr(w, "proc_error", ""):
                self.detect_var_text.set("LOI nhan dien: %s" % w.proc_error[:90])
            elif self._last_dets:
                self.detect_var_text.set("  ".join(
                    "%s n=(%+.2f,%+.2f) dt=%.2f%%"
                    % (d.label, d.nx, d.ny, d.area_frac * 100.0)
                    for d in self._last_dets[:4]))
            else:
                self.detect_var_text.set("khong thay mau nao")

        # Khong ve khi tab bi an: giai ma + doi mau la phan ton CPU nhat.
        if not self._is_visible():
            return
        if frame is None:
            self._show_message("CAMERA %s\n%s" % (w.state, w.last_error or ""))
            return
        if seq == self._last_seq:
            return                      # chua co anh moi -> khong ve lai
        self._last_seq = seq
        ...
        if self.detect_var.get():
            try:
                if self.mask_var.get():
                    # Mask la anh MOT KENH -> doi sang BGR de dung chung duong
                    # ve. Xem mask la cach chinh nguong HSV nhanh nhat: vat can
                    # tim phai trang dac, nen phai den.
                    import cv2
                    prof = next(p for p in self._detector.profiles
                                if p.name == self.mask_color_var.get())
                    m = self._detector.build_mask(frame, prof)
                    shown = cv2.cvtColor(m, cv2.COLOR_GRAY2BGR)
                elif self._last_dets:
                    shown = self._detector.draw(frame, self._last_dets)
            except Exception as exc:                      # noqa: BLE001
                self.detect_var_text.set("LOI ve overlay: %s" % exc)

        photo = self._to_photo(shown, cw, ch)
        if photo is None:
            return
        self._photo = photo             # giu tham chieu (Tk khong tu giu)
        self.canvas.delete("frame")
        self.canvas.itemconfigure(self._msg_id, text="")
        self.canvas.create_image(cw // 2, ch // 2, image=photo, anchor="center",
                                  tags="frame")

    def _to_photo(self, frame, cw, ch):
        """numpy BGR -> tk.PhotoImage, qua PPM.

        KHONG dung Pillow. Da DO tren chinh may nay (Tk 8.6.13, 320x240):
            PPM bytes tho   0.30 ms/frame   (tran ~3300 fps)
            PNG base64     13.36 ms/frame   (tran ~75 fps)
            PPM base64     KHONG CHAY  -- Tcl bao "couldn't recognize image data"
        Voi stream 12 fps thi ngan sach moi frame la 83 ms, nen 0.30 ms la
        khong dang ke. Them Pillow chi de doi lay 0.3ms nay la khong co ly do,
        ma lai them mot dependency va mot nhanh code thu hai khong ai chay.

        MAU DA KIEM, khong phai suy doan: cv2.imencode() coi dau vao la BGR va
        ghi ra RGB. Do lai bang img.get():
            BGR(0,0,255) do   -> Tk doc (255,0,0) do    OK
            BGR(255,0,0) lam  -> Tk doc (0,0,255) lam   OK
        Neu sau nay doi sang duong khac, PHAI do lai cho nay -- doi kenh do/lam
        la loi im lang, va no se pha dung phan color detection sap lam.
        """
        import cv2
        try:
            ...
            # Phong to bang so NGUYEN cho vua canvas. Tk chi co zoom nguyen
            # (nhan ban diem anh) -- tho hon noi suy cua Pillow, nhung day la
            # cua so xem truc tiep, khong phai anh de do dac. Chan o 3 de mot
            # canvas rat rong khong lam bung bo nho anh.
            ...
            self._show_message("Khong ve duoc frame: %s" % exc)
            return None
...
class PidTunerApp:
    ...
    def _build_ui(self):
        ...
        # ---- Tab Camera: TAB RIENG, khong nhet vao Config ----
        # Ve video la viec ton CPU nhat trong GUI nay. Dat rieng mot tab nghia
        # la khi nguoi dung dang o Config/Manual thi _redraw() tu ngung (xem
        # CameraPanel.is_visible) -> khong ai tra gia cho camera khi khong xem.
        camera_tab = ttk.Frame(self.notebook)
        self.notebook.add(camera_tab, text="Camera")
        self.camera_panel = CameraPanel(camera_tab, self.notebook, camera_tab,
                                         host_getter=lambda: self.host_var.get().strip())
    ...
    def _on_close(self):
        self.disconnect()
        # Thread camera la daemon nen khong chan thoat, nhung dong socket tu te
        # van hon: bo mac se de ESP32 giu mot ket noi /stream chet cho toi khi
        # TCP timeout, va max_open_sockets chi co 3.
        if getattr(self, "camera_panel", None):
    ...
    def _handle_compact_status(self, mc):
        """TELEMETRY_LEVEL=1: chi co ARM/THR/R/P/ALTm/TGT.

        Cap nhat dung nhung o do. Cac panel khac (ToF, terrain, pin, takeoff...)
        GIU NGUYEN gia tri cu thay vi bi xoa -- firmware khong gui, khong co
        nghia la gia tri do da thanh 0.
        """
        armed, thr, roll, pitch, altm, tgt = mc.groups()
        try:
            self.telemetry_var.set(
                "ARM=%s THR=%s | R=%s P=%s | [TELEMETRY MINIMAL - dat "
                "TELEMETRY_LEVEL=2 de xem day du]" % (armed, thr, roll, pitch))
            if hasattr(self, "alt_group"):
                # mode/tof/av/terr khong co trong dong gon -> truyen so trung
                # tinh thay vi doan. Nhan "MINIMAL" o tren da noi ro vi sao.
                self.alt_group.set_live(float(altm), 0.0, float(tgt), 2,
                                         0.0, 1, 0)
    ...
    def _handle_line(self, line: str):
        ...
        # Thu dinh dang GON truoc. Neu khop thi cap nhat DUNG nhung o co du
        # lieu roi thoat -- KHONG chay tiep xuong khoi FULL, va cung khong ghi
        # de cac o khac bang gia tri mac dinh (o dang giu so cuoi cung con dung
        # hon la bi xoa trang moi 50ms).
        mc = COMPACT_RE.fullmatch(line.strip())
        if mc:
            self._handle_compact_status(mc)
            return

# Graph Report - UAV-S3  (2026-09-07)

## Corpus Check
- 95 files · ~228,596 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1167 nodes · 2269 edges · 106 communities (58 shown, 48 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 326 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f4aad690`
...
## Communities (106 total, 48 thin omitted)
...
Cohesion: 0.06
Nodes (48): battery_driver_init(), battery_driver_read(), battery_driver_read_v(), battery_sample_t, esp_err_t, esp_err_t, i2c_master_bus_handle_t, imu_calib_t (+40 more)
...
Cohesion: 0.20
Nodes (24): alt_estimator_t, alt_estimator_agl_m(), alt_estimator_confirm_liftoff(), alt_estimator_correct_velocity(), alt_estimator_floor_ready(), alt_estimator_height_above_landing_surface(), alt_estimator_lock_floor(), alt_estimator_lock_floor_at_zero() (+16 more)
...
Cohesion: 0.18
Nodes (22): commander_clamp_altitude(), commander_config_t, fsm_transition(), abort_landing_to_hold(), apply_command(), finalize_mag_calibration(), recompute_uncalibrated(), fsm_state_t (+14 more)
...
### Community 19 - "clampf"
Cohesion: 0.12
Nodes (30): alt_hold_result_t, clampf(), alt_hold_default_tune(), alt_hold_preload(), alt_hold_run(), alt_hold_vz_cascade(), alt_hold_state_t, alt_hold_tune_t (+22 more)
...
Cohesion: 0.16
Nodes (14): fsm_arm_guard_ok(), fsm_state_name(), fsm_state_t, telemetry_snapshot_t, flight_core_read_telemetry(), handle_single_char(), cmd_cal_status(), cmd_heartbeat() (+6 more)
...
Cohesion: 0.12
Nodes (33): vec3f_t, vec3f_zero(), imu_driver_get_config(), apply_set_param(), imu_sample_t, TickType_t, vec3f_t, enter_kill_latch() (+25 more)
...
### Community 29 - "flight_core_tof_reinit"
Cohesion: 0.32
...
Cohesion: 0.25
Nodes (4): board_config_fill(), flight_core_board_config_t, app_main(), register_commands()
...
Cohesion: 0.17
Nodes (12): 4.1 Commander chạy ở MỌI state đã armed, 4.1b ARM — cổng vào duy nhất, và vì sao nó không còn im lặng, 4.3 Chuỗi LANDING — touchdown đa điều kiện, 4.5 Bù cos(tilt) cho throttle — feedforward, 4.7 FSM_FLYING — tắt tầng NGOÀI, giữ tầng TRONG, 4.8 BENCH_MODE — chạy full logic, KHÔNG xuất ra motor, 4. FlightStateMachine (topology), Bàn giao FLYING → HOLDING là bumpless SẴN (+4 more)
...
### Community 98 - "test_terrain_accumulate_offline.py"
Cohesion: 0.29
...
### Community 101 - "6. Telemetry an toàn/realtime — đọc gì khi soi log"
Cohesion: 0.33
...
### Community 102 - "4.4 Altitude estimator — accel-primary, ToF CHỈ LÀ correction"
Cohesion: 0.40
...
### Community 105 - "4.6 Terrain offset — bay qua bàn/ghế mà không mất tham chiếu"
Cohesion: 0.67
Nodes (3): 4.6 Terrain offset — bay qua bàn/ghế mà không mất tham chiếu, Guard khoảng hở tối thiểu (B8) — lưới an toàn CUỐI, ⚠ Vòng luẩn quẩn `terr_pending` — lỗi đã làm feature này bị TẮT

## Knowledge Gaps
- **77 isolated node(s):** `graphify`, `graphify`, `Trạng thái hiện tại (đọc mục này trước)`, `Kiến trúc: ĐÚNG 2 tầng`, `Vì sao C (không phải C++) cho tầng dưới` (+72 more)
...
- **Why does `flight_core_start()` connect `flight_core_start` to `sensor_hub.c`, `fc_module.c`, `mahony_filter.c`, `tof_driver.c`, `stabilize_task`, `apply_command`, `baro_driver.c`, `mag_driver.c`, `app_config.h`, `clampf`, `attitude_control_update`, `calibration.c`, `flight_core.c`, `motor_driver.c`, `commander_evaluate`, `types.h`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `stabilize_task()` connect `stabilize_task` to `sensor_hub.c`, `mahony_filter.c`, `sensor_hub_age_us`, `hover_model.c`, `apply_command`, `baro_driver.c`, `clampf`, `attitude_control_update`, `calibration.c`, `flight_core.c`, `motor_driver.c`, `commander_evaluate`, `types.h`, `flight_core_start`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `apply_command()` connect `apply_command` to `mahony_filter.c`, `hover_model.c`, `flight_core_push_command`, `stabilize_task`, `baro_driver.c`, `mag_driver.c`, `flight_core_tof_reinit`, `clampf`, `flight_core_read_telemetry`, `calibration.c`, `flight_core.c`, `motor_driver.c`, `types.h`, `flight_core_start`?**
